# =============================================================================
# 06_backtest_mar2_vs_var2.R
# Rolling-window walk-forward backtest: MAR(2) vs VAR(2, vectorized 25-dim)
# For exploratory purposes only — VAR(2) is underdetermined on ~305 obs
...
cat("NOTE: VAR(2) has 1250 parameters on", train_size, "obs — expect instability\n\n")
...
for (w in seq_len(n_windows)) {
  ...
  # ==== VAR(2) — vectorized 25-dim ====
  Y_train <- t(apply(xx_train, 1, function(sl) as.vector(sl)))
  
  var_fit <- tryCatch(
    VAR(Y_train, p = 2, type = "none"),
    error   = function(e) NULL,
    warning = function(w) NULL
  )
  
  if (!is.null(var_fit)) {
    pred_var_vec <- tryCatch(
      sapply(predict(var_fit, n.ahead = 1)$fcst, function(x) x[1, "fcst"]),
      error = function(e) rep(0, N_series)
    )
  } else {
...
cat("\nBacktest complete.\n")
cat("VAR(2) estimation failures:", var_failures, "/", n_windows, "\n")
...
results_df <- data.frame(
  ...
  RMSE_VAR2  = round(rmse_var, 4),
  RMSE_ratio = round(rmse_mar / rmse_var, 4)
)

cat("\n=== AGGREGATE FROBENIUS MSFE ===\n")
cat(sprintf("MAR(2): %.4f\n", msfe_mar))
cat(sprintf("VAR(2): %.4f\n", msfe_var))
cat(sprintf("MSFE ratio (MAR2/VAR2): %.4f [<1 = MAR wins]\n", msfe_mar / msfe_var))
...
dm_result <- dm.test(e1 = sqrt(frob_mar),
                     ...
                     alternative = "less",
                     h = 1, power = 2)
cat("\n=== DIEBOLD-MARIANO TEST (H1: MAR(2) < VAR(2)) ===\n")
print(dm_result)

# ── SAVE ──────────────────────────────────────────────────────────────────────
write.csv(results_df,
          paste0(results_dir, "per_series_rmse_mar2_vs_var2.csv"),
          row.names = FALSE)
...
saveRDS(list(msfe_mar  = msfe_mar, msfe_var  = msfe_var,
        ...
        paste0(results_dir, "backtest_results_mar2_vs_var2.rds"))

cat("\nResults saved to:", results_dir, "\n")

# ── PLOTS ─────────────────────────────────────────────────────────────────────
png(paste0(results_dir, "rolling_loss_mar2_vs_var2.png"),
...
plot(frob_mar, type = "l", col = "steelblue", lwd = 1.5,
     ...
     main = "Rolling OOS Loss: MAR(2) vs VAR(2) [exploratory]")
lines(frob_var, col = "firebrick", lwd = 1.5)
legend("topright", legend = c("MAR(2)", "VAR(2)"),
       col = c("steelblue", "firebrick"), lwd = 2, bty = "n")
dev.off()

png(paste0(results_dir, "rmse_ratio_mar2_vs_var2.png"),
...
barplot(results_df$RMSE_ratio,
        ...
        col = colors,
        ylab = "RMSE Ratio (MAR(2) / VAR(2))",
        main = "Per-Series RMSE Ratio: MAR(2) / VAR(2)\n[Blue <1 = MAR wins, Red >1 = VAR wins]")
